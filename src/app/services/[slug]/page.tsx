  ...
  if (!service) {
    return (
      <div className="py-20 max-w-7xl mx-auto px-4 text-center">
        <h2 className="text-2xl font-serif font-bold text-[#14402a]">Service Not Found</h2>
        <Link href="/services" className="px-6 py-3 rounded-full bg-[#14402a] text-white text-xs font-bold mt-4 inline-block">
          Back to Services
        </Link>
      </div>
    );
  ...
  return (
    <div className="py-8 sm:py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-xs font-medium text-[#52685a] mb-6">
        ...
        <span>/</span>
        <Link href="/services" className="hover:text-[#14402a]">Services</Link>
        <span>/</span>
        <span className="text-[#14402a] font-semibold">{service.title}</span>
      </div>

      {/* Hero Banner */}
      <div className="relative rounded-3xl overflow-hidden shadow-2xl bg-[#14402a] mb-12">
        <div className="relative aspect-[21/9] min-h-[320px] w-full">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#14402a] via-[#14402a]/60 to-transparent" />
          
          <div className="absolute inset-0 p-6 sm:p-12 flex flex-col justify-end max-w-3xl text-white space-y-3">
            <span className="text-[11px] sm:text-xs font-bold uppercase tracking-[0.2em] text-[#d47343] bg-black/30 px-3 py-1 rounded-full self-start">
              {service.pricingRange}
            </span>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-serif font-extrabold text-white">
              {service.title}
            </h1>
            <p className="text-xs sm:text-sm text-[#d4e4db] max-w-2xl leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-7 space-y-12">
          ...
          <div className="space-y-4">
            <h3 className="text-2xl font-serif font-bold text-[#14402a]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {service.benefits.map((b, idx) => (
                <div key={idx} className="bg-white rounded-2xl p-5 border border-[#e5ece3] shadow-xs space-y-2">
                  <div className="w-8 h-8 rounded-full bg-[#eaf0e7] flex items-center justify-center text-[#14402a] font-bold text-xs">
                    0{idx + 1}
                  </div>
          ...
          <div className="bg-[#eaf0e7] rounded-3xl p-6 sm:p-8 space-y-6">
            <div>
              ...
              <h3 className="text-xl sm:text-2xl font-serif font-bold text-[#14402a] mt-1">
            ...
            <div className="space-y-4">
              {service.process.map((step) => (
                <div key={step.step} className="flex items-start gap-4 bg-white rounded-2xl p-4 shadow-xs">
                  <div className="w-9 h-9 rounded-xl bg-[#14402a] text-white flex items-center justify-center shrink-0 font-serif font-bold text-sm">
                    {step.step}
                  </div>
                  <div>
          ...
          {/* FAQs */}
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#e5ece3] shadow-md space-y-4">
            <h3 className="text-xl font-serif font-bold text-[#14402a] pb-3 border-b border-[#f0f4ee]">
              ...
              {service.faqs.map((faq, idx) => (
                <div key={idx} className="border border-[#edf3ec] rounded-2xl overflow-hidden">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-4 h-4 text-[#14402a] transition-transform ${openFaq === idx ? 'rotate-180' : ''}`} />
                  </button>
                  {openFaq === idx && (
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl p-6 sm:p-8 border border-[#e5ece3] shadow-xl space-y-6 sticky top-24">
          <div className="pb-4 border-b border-[#f0f4ee]">
            ...
            <h3 className="text-xl font-serif font-bold text-[#14402a] mt-2">
          ...
          {isSubmitted ? (
            <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-200 text-center space-y-3">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#14402a] text-white text-xs font-bold hover:bg-[#1b5539]"
              >
